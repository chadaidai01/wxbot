...
_wx = None
_wx_available = False
_wx_init_attempted = False
_wx_init_lock = threading.Lock()
UNSENT_LOG_FILE = "unsent_replies.log"

# 生成用户昵称列表和prompt映射字典
user_names = [entry[0] for entry in LISTEN_LIST]
prompt_mapping = {entry[0]: entry[1] for entry in LISTEN_LIST}

# 每个聊天/群的"剧场/全局阅读"开关：LISTEN_LIST 第3个元素为真即开启。
# 开启后该聊天（尤其群聊）消息全局读取并进入剧场叙事，回复参考最近聊天记录自然接话。
_THEATER_CHATS = set()
for _entry in LISTEN_LIST:
    if len(_entry) >= 3 and _entry[2] in (True, 1, '1', 'true', 'True', 'yes', 'on', '是', '开', '开启'):
        _THEATER_CHATS.add(_entry[0])


def is_theater_chat(user):
    """该聊天（私聊/群）是否开启剧场/全局阅读模式。"""
    if not ENABLE_THEATER:
        return False
    if user in _THEATER_CHATS:
        return True
    return _LOCALDB_ALIASES.get(user) in _THEATER_CHATS
    ...
    try:
        ...
        if match:
            ...
            if value_str.lower() in ('true', 'false'):
                ...
                # 用 ast.literal_eval 安全求值：只解析 Python 字面量，不执行任意代码，
                # 避免 eval() 的代码注入风险（config.py 可能被网页编辑器/导入功能改写）。
                try:
...
queue_lock = threading.RLock()  # 队列/上下文访问锁（可重入：允许同一线程在持锁时再调用加锁的辅助函数）
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def close(self):
        ...
        if not self.log_queue.empty():
            ...
            # 注意：queue.Queue.join() 不接受 timeout 参数（旧写法 join(timeout=30) 会抛
            # TypeError 被静默吞掉，根本没等待）。这里改用有界轮询，最多等待 30 秒排空队列。
...
# 获取登录用户的名字（用于群聊@识别；wxauto 初始化成功后会更新为微信昵称）
ROBOT_WX_NAME = BOT_NICKNAME or ''


def _init_wx_inner():
    """在子线程中真正调用 wxauto（可被 ensure_wx 超时放弃）。"""
    global _wx, ROBOT_WX_NAME
    from wxautox4_wechatbot import WeChat
    _wx = WeChat()
    if getattr(_wx, 'nickname', None):
        ROBOT_WX_NAME = _wx.nickname
    return True


def ensure_wx():
    """懒初始化 wxauto（仅用于发送）。带超时保护，失败不阻塞，返回是否可用。"""
    global _wx, _wx_available, _wx_init_attempted, ROBOT_WX_NAME
    WX_INIT_TIMEOUT = 20.0  # 微信接口初始化超时（秒）
    with _wx_init_lock:
        if _wx_init_attempted:
            return _wx_available
        _wx_init_attempted = True
        result = {}
        def runner():
            try:
                result['ok'] = _init_wx_inner()
            except Exception as e:
                result['err'] = e
        t = threading.Thread(target=runner, daemon=True)
        t.start()
        t.join(timeout=WX_INIT_TIMEOUT)
        if 'ok' in result:
            _wx_available = True
            logger.info(f"\033[32m微信发送通道就绪，机器人昵称: {ROBOT_WX_NAME}\033[0m")
        else:
            err = result.get('err', '初始化超时（微信窗口无响应）')
            _wx = None
            _wx_available = False
            logger.warning(f"微信发送通道(wxauto)初始化失败: {err}")
            logger.warning(f"机器人仍会通过 WeFlow 监听并生成回复；发送通道不可用，回复将只记录到 {UNSENT_LOG_FILE}。")
        return _wx_available


def _log_unsent(who, content, kind="文本"):
    log_path = os.path.join(root_dir, UNSENT_LOG_FILE)
    try:
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 未发送{kind} -> {who}: {content}\n")
    except Exception:
        pass
    logger.warning(f"[发送通道不可用] 未发送{kind}给 {who}: {str(content)[:100]}")


_wechat_hwnd_cache = []
_wechat_hwnd_cache_ts = 0.0


def _ensure_wechat_focus():
    """发送前把微信主窗口带到前台并恢复，确保 Ctrl+A/回车等快捷键可用。"""
    global _wechat_hwnd_cache, _wechat_hwnd_cache_ts
    try:
        ...
        # 每 5 秒重新枚举一次窗口（句柄可能变化）
        if not _wechat_hwnd_cache or time.time() - _wechat_hwnd_cache_ts > 5:
            found = []
            def _cb(hwnd, _lparam):
                ...
                if (title in ('微信', 'Weixin', 'WeChat')
                        or '微信' in title or 'Weixin' in title
                        or 'WeChat' in cls or 'Weixin' in cls
                        or (ROBOT_WX_NAME and ROBOT_WX_NAME in title)):
                    found.append(hwnd)
                return True
            WNDENUMPROC = ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.c_void_p, ctypes.c_void_p)
            user32.EnumWindows(WNDENUMPROC(_cb), 0)
            _wechat_hwnd_cache = found
            _wechat_hwnd_cache_ts = time.time()

        for hwnd in _wechat_hwnd_cache:
            user32.ShowWindow(hwnd, SW_RESTORE)
            user32.SetWindowPos(hwnd, HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW)
            user32.SetForegroundWindow(hwnd)
        return bool(_wechat_hwnd_cache)
    except Exception as e:
        logger.warning(f"聚焦微信窗口失败: {e}")
        return False


def _send_msg(who, content):
    """发送文本消息；发送通道不可用时降级为记录到文件。"""
    try:
        if not _wx_available and not ensure_wx():
            _log_unsent(who, content)
            return False
        _ensure_wechat_focus()
        return _wx.SendMsg(msg=content, who=who)
    except Exception as e:
        logger.warning(f"发送文本给 {who} 失败: {e}")
        return False


def _send_file(who, filepath):
    """发送文件/表情包；发送通道不可用时降级为记录。"""
    try:
        if not _wx_available and not ensure_wx():
            _log_unsent(who, filepath, kind="文件")
            return False
        _ensure_wechat_focus()
        return _wx.SendFiles(filepath=filepath, who=who)
    except Exception as e:
        logger.warning(f"发送文件给 {who} 失败: {e}")
        return False


def _voice_call(who):
    """发起语音通话（提醒用）；发送通道不可用时降级为记录。"""
    try:
        if not _wx_available and not ensure_wx():
            _log_unsent(who, "[语音通话]", kind="通话")
            return False
        _ensure_wechat_focus()
        return _wx.VoiceCall(who)
    except Exception as e:
        logger.warning(f"语音通话给 {who} 失败: {e}")
        return False
...
def _theater_image_recognize(image_path):
    """剧场识图回调：识别图片/表情，返回纯文字描述（供 AI 参考）。"""
    try:
        text = recognize_image_with_moonshot(image_path, is_emoji=False)
        text = (text or '').strip()
        if '：' in text:
            text = text.split('：', 1)[1].strip()
        return text
    except Exception as e:
        logger.warning(f"剧场识图失败: {e}")
        return ''
...
# 微信窗口操作互斥锁：发送消息期间持有，防止后台线程抢焦点导致 Ctrl+A 失效、狂打 a
wx_op_lock = threading.Lock()

# 用于拍一拍功能的全局变量
user_last_msg = {}  # {user_id: msg对象} 存储每个用户最后发送的消息对象
bot_last_sent_msg = {}  # 存储机器人发送给每个用户的最后一条消息（用于撤回/拍一拍自己）
...
def is_user_group_chat(user_name):
    """
    检查指定用户是否为群聊（群聊类型来自本地数据监听器缓存的会话信息）。
    ...
    with group_cache_lock:
        # 缓存由本地数据监听器(LocalDbListener.on_sessions)维护
        return group_chat_cache.get(user_name, False)

# 本地数据监听下，会话显示名(display_name) -> 配置昵称 的别名映射（用于私聊过滤/群聊功能匹配）
_LOCALDB_ALIASES = {}

def update_group_chat_cache_from_sessions(sessions_map):
    """
    根据本地数据监听器返回的会话信息更新群聊缓存与显示名别名。
    
    Args:
        sessions_map (dict): {display_name: {username, display_name, is_group, ...}}
    """
    global group_chat_cache, _LOCALDB_ALIASES
    try:
        with group_cache_lock:
            for display_name, info in sessions_map.items():
                if isinstance(info, dict):
                    group_chat_cache[display_name] = bool(info.get('is_group'))
            # 建立配置昵称 <-> 会话显示名 的别名映射（精确/部分匹配）
            new_aliases = {}
            for user_name in user_names:
                if user_name in sessions_map:
                    info = sessions_map[user_name]
                    if isinstance(info, dict):
                        group_chat_cache[user_name] = bool(info.get('is_group'))
                    new_aliases[user_name] = user_name
                else:
                    matched = None
                    for display_name, info in sessions_map.items():
                        ...
                        if info.get('username') == user_name:
                            ...
                            break
                    if matched is None:
                        for display_name in sessions_map:
                            if display_name and user_name and (
                                    user_name in display_name or display_name in user_name):
                                matched = display_name
                                break
                    if matched:
                        new_aliases[user_name] = matched
            _LOCALDB_ALIASES = {v: k for k, v in new_aliases.items()}
            logger.debug(f"已根据本地会话更新群聊缓存与别名，共 {len(group_chat_cache)} 条会话")
    except Exception as e:
        logger.error(f"更新群聊缓存(本地会话)失败: {e}", exc_info=True)
...
def load_chat_contexts():
    ...
    data_path = os.path.join(root_dir, CHAT_CONTEXTS_FILE)
    try:
        if os.path.exists(data_path):
            with open(data_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(loaded_contexts, dict):
                    ...
                    logger.info(f"成功从 {data_path} 加载 {len(chat_contexts)} 个用户的聊天上下文。")
                else:
                    logger.warning(f"{data_path} 文件内容格式不正确（非字典），将使用空上下文。")
                    chat_contexts = {} # 重置为空
        else:
            logger.info(f"{data_path} 未找到，将使用空聊天上下文启动。")
            chat_contexts = {} # 初始化为空
    except json.JSONDecodeError:
        logger.error(f"解析 {data_path} 失败，文件可能已损坏。将使用空上下文。")
        chat_contexts = {} # 重置为空
    except Exception as e:
...
def _content_to_plain_text(content):
    """把 content（str 或 多模态 list）转为纯文本，用于日志/合并。"""
...
def merge_context(context_list):
    ...
    兼容 content 为 str 或 多模态 list 的情况。
    """
    if not context_list:
        return []
    merged = []
    ...
    for item in context_list:
        ...
        if role == last_role:
            ...
            if buffer:
                merged.append({'role': last_role, 'content': '\n'.join(_content_to_plain_text(c) for c in buffer)})
            buffer = [content]
            last_role = role
    if buffer:
        merged.append({'role': last_role, 'content': '\n'.join(_content_to_plain_text(c) for c in buffer)})
...
def save_chat_contexts():
    """将当前聊天上下文保存到文件（跨进程原子写：FileLock + 唯一 tmp + os.replace + fsync）。"""
    global chat_contexts
    data_path = os.path.join(root_dir, CHAT_CONTEXTS_FILE)
    lock_path = data_path + '.lock'  # 与 config_editor 使用同一把锁，避免两进程同时写互相截断
    temp_file_path = f"{data_path}.{os.getpid()}.{uuid.uuid4().hex}.tmp"
    try:
        # 创建要保存的上下文副本，以防在写入时被其他线程修改
        contexts_to_save = dict(chat_contexts) # 创建浅拷贝
        # --- 保存前合并每个用户的上下文，保证 user/assistant 交替 ---
        ...
        with FileLock(lock_path):
            with open(temp_file_path, 'w', encoding='utf-8') as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            os.replace(temp_file_path, data_path)  # Windows 上原子替换（shutil.move 非原子）
        logger.debug(f"聊天上下文已成功保存到 {data_path}")
    except Exception as e:
        logger.error(f"保存聊天上下文到 {data_path} 失败: {e}", exc_info=True)
    finally:
        # 失败路径清理临时文件（成功后已被 os.replace 重命名，不存在）
...
def get_deepseek_response(message, user_id, store_context=True, is_summary=False, image_paths=None):
    """
    ...
    参数:
        ...
        image_paths (list, optional): 图片本地路径列表。非空时构造多模态消息，
                              图片直接发给 Chat 模型原生识别。
    """
    try:
        # 注意：聊天上下文的重新加载已移入下方 queue_lock 临界区，避免在锁外重绑全局
        # chat_contexts 字典对象造成并发丢更新（多线程/主动消息线程同时处理时）。
        ...
        if image_paths:
            for p in image_paths:
                try:
                    ...
                    continue
        has_images = bool(valid_image_paths)

        # 构建当前用户消息 content（支持多模态）
        if has_images:
            current_user_content = []
            if message and message.strip():
                current_user_content.append({"type": "text", "text": message})
            for img_p in valid_image_paths:
                current_user_content.append({"type": "image_url", "image_url": {"url": img_p, "_local_path": True}})
        else:
            current_user_content = message

        logger.info(f"调用 Chat API - ID: {user_id}, 是否存储上下文: {store_context}, 是否含图片: {has_images}, 消息: {(message[:100] if isinstance(message, str) else '')}...") # 日志记录消息片段
        ...
        if store_context:
            ...
            with queue_lock: # 确保对 chat_contexts 的访问是线程安全的
                # 在锁内重新加载，使"加载→读历史→追加用户消息→保存"成为原子操作
                load_chat_contexts()
                ...
                messages_to_send.append({"role": "user", "content": current_user_content})

                # 4. 在准备 API 调用后更新持久上下文
                # 将用户消息添加到持久存储中
                chat_contexts[user_id].append({"role": "user", "content": current_user_content})
...
def _local_image_to_data_url(path):
    """读取本地图片文件并转成 base64 data URL。失败返回 None。"""
    try:
        ...
        logger.error(f"读取本地图片转 data URL 失败: {path}, 错误: {e}")
        return None


def _convert_local_images_to_data_urls(messages_to_send):
    """把消息中的本地图片路径（_local_path 标记）转换为 data URL，兼容 str/list content。"""
        ...
        if isinstance(content, list):
            ...
            for part in content:
                if isinstance(part, dict) and part.get('type') == 'image_url':
                    iurl = part.get('image_url') or {}
                    url = iurl.get('url') if isinstance(iurl, dict) else None
                    if iurl.get('_local_path') and url:
                        data_url = _local_image_to_data_url(url)
                        if data_url:
...
def call_chat_api_with_retry(messages_to_send, user_id, max_retries=2, is_summary=False, model=None):
    """
    ...
    返回:
        str: API 返回的文本回复。
    """
    # 发送前把消息里的本地图片路径转换为 data URL（原生多模态）
    messages_to_send = _convert_local_images_to_data_urls(messages_to_send)
    ...
    while attempt <= max_retries:
        try:
            ...
            # 如果到这里说明内容为空或过滤后为空（剧场模式由 _fallback_reply 兜底，降级为 warning）
            if response.choices and response.choices[0].message.content is not None:
                if user_id == 'theater':
                    logger.warning(f"剧场叙事返回了空内容或内容被过滤，将走兜底回复。")
                else:
                    logger.error(f"API返回了空的内容或内容被过滤 (ID: {user_id})")
            logger.warning(f"错误请求消息体模型: {model or MODEL} (ID: {user_id})")
        ...
        # 重试前指数退避（封顶 8s）：避免对限流/繁忙的 API 立即重试而加剧问题
...
# --- 自消息识别（可靠方案）---
# 库判定消息是 self 还是 friend 靠的是气泡左右方向，长时间运行会漂移，
# 而且群聊里机器人自己的消息 sender 是群昵称（不等于账号昵称 ROBOT_WX_NAME），
# 因此既不能用 attr 也不能用 sender 可靠区分。这里改为：记录机器人每次发送
# 出去的文本内容，收到消息时若内容匹配最近发送的文本，即判定为机器人自己发的消息。
_recent_sent_texts = {}   # {text: timestamp}
_recent_sent_lock = threading.Lock()
SELF_MSG_MATCH_WINDOW = 180   # 匹配窗口（秒）
SELF_MSG_MAX_CACHE = 300      # 缓存上限

def record_sent_text(text):
    """记录机器人即将/已经发送出去的文本内容。"""
    if not text:
        return
    text = text.strip()
    ...
    with _recent_sent_lock:
        _recent_sent_texts[text] = now
        # 清理过期的记录
        expired = [t for t, ts in _recent_sent_texts.items() if now - ts > SELF_MSG_MATCH_WINDOW]
        for t in expired:
            _recent_sent_texts.pop(t, None)
        # 限制缓存大小，超出则删除最旧的
        if len(_recent_sent_texts) > SELF_MSG_MAX_CACHE:
            oldest = sorted(_recent_sent_texts.items(), key=lambda x: x[1])
            for t, _ in oldest[:len(_recent_sent_texts) - SELF_MSG_MAX_CACHE]:
                _recent_sent_texts.pop(t, None)

def is_self_sent_text(text):
    """判断给定文本是否匹配机器人最近发送过的内容。"""
    if not text:
        return False
    text = text.strip()
    if not text:
        return False
    with _recent_sent_lock:
        return text in _recent_sent_texts


def message_listener(msg, chat):
    global can_send_messages, user_last_msg, bot_last_sent_msg, BOMBING_ENABLED
    who = chat.who 
    msgtype = msg.type
    ...
    # 全局停止指令：任何群/私聊，任何人发 /stop（或 /停止、/shutdown）即关闭 bot
    if msgtype == 'text' and original_content:
        stripped_cmd = str(original_content).strip()
        if stripped_cmd in ('/stop', '/停止', '/shutdown', '停止'):
            logger.warning(f"收到停止指令：{who} 发送了 {stripped_cmd}，bot 即将关闭...")
            try:
                wx_op_lock.acquire()
                try:
                    record_sent_text('好的，我先休息啦，bye~')
                    _send_msg(who, '好的，我先休息啦，bye~')
                finally:
                    wx_op_lock.release()
            except Exception as _e:
                logger.warning(f"发送停止回复失败: {_e}")
            time.sleep(1)
            os._exit(0)

    # --- 自消息识别（可靠）：内容匹配优先 ---
    # 机器人自己刚发送的文本，库可能误判为 friend（气泡方向漂移），
    # 这里用内容匹配识别，命中则按 self 处理，避免机器人回复自己的消息。
    if msgtype == 'text' and is_self_sent_text(original_content):
        logger.info(f"[自消息识别] 消息内容匹配机器人最近发送的文本，判定为机器人自己发送的消息，忽略。")
    ...
    if msgattr == 'tickle':
        if "我拍了拍" in original_content:
            ...
            if ENABLE_AUTO_PAT_BACK:
                try:
                    if hasattr(msg, 'tickle'):
                        msg.tickle()
                        logger.info(f"[自动回拍] 已回拍 {sender or who}")
                    elif who in user_last_msg and user_last_msg[who]:
                        user_last_msg[who].tickle()
                        logger.info(f"[自动回拍] 已回拍 {who}")
                    else:
                        logger.info(f"[自动回拍] 未找到可回拍的对象（{who}）")
                except Exception as e:
                    logger.error(f"自动回拍失败: {e}")
            return  # 拍一拍消息不进入AI对话处理
    elif msgattr == 'self':
        # 保存机器人自己发送的消息，用于拍一拍自己功能
        bot_last_sent_msg[who] = msg
        # 自己账号发 /轰炸 开启轰炸，/轰炸停 关闭轰炸（手动指令，优先处理）
        self_content = (original_content or '').strip()
        if self_content == '/轰炸':
            if ENABLE_BOMBING:
                BOMBING_ENABLED = True
                logger.info(f"[轰炸] 已通过self指令开启轰炸（目标: {BOMBING_USER}）")
            else:
                logger.info("[轰炸] 功能未在网页中开启，忽略 /轰炸 指令")
            return
        if self_content in ('/轰炸停', '/轰炸停止', '/轰炸关'):
            BOMBING_ENABLED = False
            logger.info("[轰炸] 已通过self指令关闭轰炸")
            return
        # 本地监听(WeFlow)的 isSend 标志可靠：attr=='self' 即机器人自己发的消息。
        # 上方"内容匹配"已拦截最近发送的文本；走到这里的多为更早的、或从其他设备发出的
        # 自消息，同样应按 self 处理并直接返回——绝不能回灌给 AI，否则机器人会回复自己的
        # 历史消息形成自言自语。（旧逻辑此处会"按好友消息继续处理"，在本地监听下是错误的。）
        logger.debug(f"已保存机器人发送给 {who} 的最后消息对象（self），不进入 AI 处理")
        return  # 不处理机器人自己的消息
    elif msgattr == 'system':
        # 自动回拍：system 消息完整包含"我的拍一拍"文字时，回拍拍你的人
        if ENABLE_AUTO_PAT_BACK and MY_PAT_PHRASE and MY_PAT_PHRASE in original_content:
            try:
                m = re.search(r'"([^"」]+)"\s*拍了拍', original_content)
                pat_name = m.group(1).strip() if m else ''
                if not pat_name:
                    pat_name = sender
                if pat_name and pat_name not in ('我', '你'):
                    tickled = False
                    # 1) 尝试直接tickle
                    try:
                        if hasattr(msg, 'tickle'):
                    ...
                    if not tickled:
                        try:
                            for uid, umsg in list(user_last_msg.items()):
                                if umsg and getattr(umsg, 'sender', '') == pat_name:
                                    umsg.tickle()
                                    tickled = True
                                    break
                        except Exception:
                            pass
                    # 3) 从聊天窗口读取拍你的人的消息再tickle
                    if not tickled:
                        try:
                            msgs = chat.GetAllMessage()
                            if msgs:
                                for fm in reversed(msgs):
                                    if getattr(fm, 'attr', '') == 'friend' and getattr(fm, 'sender', '') == pat_name:
                                        fm.tickle()
                                        tickled = True
                                        break
                        except Exception as e:
                            logger.error(f"自动回拍-窗口查找失败: {e}")
                    logger.info(f"[自动回拍] 已回拍 {pat_name} (成功={tickled})")
            except Exception as e:
                logger.error(f"自动回拍处理失败: {e}")
        return  # system消息不进入AI对话处理
    elif msgattr != 'friend':
        logger.info(f"非好友消息，已忽略。")
        return

    # 兜底修复：库长时间运行后偶发会把机器人自己发的消息误判为 friend。
    # 机器人自己发的消息 sender 一定是机器人昵称，据此过滤，避免把 AI 刚发的
    # 内容当成用户消息回灌，导致 AI 误以为用户在复制它。
    if sender and sender == ROBOT_WX_NAME:
        logger.info(f"[自消息过滤] 消息发送者为机器人昵称 {ROBOT_WX_NAME}，判定为机器人自己发送的消息被误判，忽略。")
        return
    ...
    # 复读机：命中复读配置则直接复读（文字/表情/图片/视频，不受群聊回复概率影响）
    if is_group_chat and REPEATER_LIST:
        for rp in REPEATER_LIST:
            if who == rp.get('group') and sender == rp.get('target'):
                try:
                    wx_op_lock.acquire()
                    try:
                        if msgtype == 'text' and original_content:
                            record_sent_text(original_content)
                            _send_msg(who, original_content)
                            logger.info(f"[复读] 群[{who}] 文字: {original_content[:30]}")
                        elif msgtype in ('emotion', 'image', 'video', 'file'):
                            msg.forward(who)
                            logger.info(f"[复读] 群[{who}] {msgtype}: {sender}")
                        else:
                            msg.forward(who)
                            logger.info(f"[复读] 群[{who}] {msgtype}: {sender}")
                    finally:
                    ...
                    logger.error(f"[复读] 失败: {e}")
                return

    # 今日小猪：命中触发词则抽一只猪（群聊/私聊均可，每人每天一次）
    if ENABLE_ROLLPIG and msgtype == 'text' and original_content and any(
            t in original_content for t in ("今日小猪", "抽小猪", "我的小猪", "rollpig",
                                            "小猪列表", "全部小猪", "猪猪列表",
                                            "小猪图鉴", "我的猪圈", "我的收藏", "解锁小猪", "猪圈")):
        try:
            import rollpig
            text, img = rollpig.handle(who, sender, original_content)
            wx_op_lock.acquire()
            try:
                if text:
                    record_sent_text(text)
                    _send_msg(who, text)
                if img:
                    _send_file(who, str(img))
            finally:
                wx_op_lock.release()
            logger.info(f"[今日小猪] {sender} 抽取完成")
        except Exception as e:
            logger.error(f"[今日小猪] 处理失败: {e}")
        return

    # 崩坏星穹铁道：命中星铁指令则处理
    if ENABLE_STARRAIL and msgtype == 'text' and original_content:
        try:
            import starrail.main as starrail_main
            result = starrail_main.handle(who, sender, original_content)
            if result is not None:
                ...
                try:
                    ...
                    wx_op_lock.release()
                logger.info(f"[星铁] {sender} 指令处理完成")
                return
        except Exception as e:
            logger.error(f"[星铁] 处理失败: {e}")

    if not is_group_chat: 
        if who in user_names or _LOCALDB_ALIASES.get(who) in user_names:
        ...
        if not ACCEPT_ALL_GROUP_CHAT_MESSAGES and ENABLE_GROUP_AT_REPLY and ROBOT_WX_NAME:
            ...
            if at_triggered:
                ...
                msg.force_reply = True  # 被 @ 了：必须回复，禁止沉默/延迟
        ...
        if is_theater_chat(who):
            ...
            if at_triggered:
                should_process_this_message = True
                logger.info(f"群聊 '{who}' 消息因 @机器人 强制回复。")
...
def recognize_image_with_moonshot(image_path, is_emoji=False):
    """使用 AI 识别图片内容并返回文本（直接调用 OpenAI 兼容接口）。"""
    global can_send_messages
    can_send_messages = False  # 先暂停向API发送消息队列

    try:
        processed_image_path = image_path

        # 读取图片内容并编码
        # 读取图片内容并编码（gif/webp/bmp 先转成 JPEG，保证识图接口兼容）
        ...
        if ext in ('.gif', '.webp', '.bmp'):
            try:
                ...
                import io
                im = Image.open(processed_image_path).convert('RGB')
                buf = io.BytesIO()
        ...
        # 选择图片识别使用的 API 配置：开启共用开关则使用 Chat 模型的配置，否则使用 Moonshot 独立配置
        if ENABLE_IMAGE_USE_CHAT_API:
            image_api_key = DEEPSEEK_API_KEY
            image_base_url = DEEPSEEK_BASE_URL
            image_model = MODEL
            image_temperature = TEMPERATURE
        else:
            image_api_key = MOONSHOT_API_KEY
            image_base_url = MOONSHOT_BASE_URL
            image_model = MOONSHOT_MODEL
            image_temperature = MOONSHOT_TEMPERATURE
        ...
        response = requests.post(f"{image_base_url}/chat/completions", headers=headers, json=data, timeout=120)
...
def handle_wxauto_message(msg, who):
    ...
    try:
        ...
        # 如果需要进行图片/表情识别
        if img_path:
            # 原生多模态：开启时（且为图片、非表情）直接把原图路径传给 Chat 模型看图，
            # 免去"识别成文字描述"的中转；表情仍走截图+文字识别。
            if ENABLE_IMAGE_NATIVE_MULTIMODAL and ENABLE_IMAGE_USE_CHAT_API and not is_emoji:
                image_paths_for_ai.append(str(img_path))
                # 保留用户原始文本（若有），否则用图片占位；图片路径随队列传给模型
                processed_content = original_content if original_content else "[图片]"
                can_send_messages = True
                logger.info(f"原生多模态：图片 {img_path} 将直接交给 Chat 模型识别")
            else:
        ...
        if processed_content:
            ...
            force_reply = bool(getattr(msg, 'force_reply', False))
            ...
            with queue_lock:
                ...
                if username not in user_queues:
                    user_queues[username] = {
                        ...
                        'image_paths': list(image_paths_for_ai),
                    ...
                    user_queues[username].setdefault('image_paths', []).extend(image_paths_for_ai)
                    if force_reply:
                        user_queues[username]['force_reply'] = True
                    # 更新最后消息时间戳
    ...
    while True:
        ...
        for username in inactive_users:
            try:
                ...
                # 关键：隔离并记录异常，避免单个用户处理失败把整个检查线程打死。
                # 否则该守护线程一旦退出，bot 会静默停止回复所有人，直到重启才恢复。
                logger.error(f"处理用户 {username} 消息时发生未捕获异常（已隔离，检查线程继续运行）: {e}", exc_info=True)
...
def process_user_messages(user_id):
    ...
    with queue_lock:
        ...
        username = user_data['username'] # username ������Ⱥ����������ǳ�
        image_paths = user_data.get('image_paths', []) or []
        force_reply = bool(user_data.get('force_reply', False))

    # 合并消息
    merged_message = ' '.join(messages)
    logger.info(f"开始处理用户 '{sender_name}' (ID: {user_id}) 的合并消息: {merged_message[:100]}...")
    if image_paths:
        logger.info(f"本次消息携带 {len(image_paths)} 张图片，将原生交给 Chat 模型识别。")
...
"""
                    # 调用主 AI 生成最终回复，存储上下文
                    reply = get_deepseek_response(final_prompt, user_id, store_context=True, image_paths=image_paths)
        ...
        if reply is None: # 只有在尚未通过联网逻辑生成回复时才执行
            ...
            reply = get_deepseek_response(merged_message, user_id, store_context=True, image_paths=image_paths)
        ...
        # 图片用完即删：识别完成后清理本次携带的临时图片
        for img_p in image_paths:
            try:
                if img_p and os.path.exists(img_p) and ('wxautox文件下载' in img_p):
                    os.remove(img_p)
                    logger.info(f"[图片清理] 已删除临时图片: {img_p}")
            except Exception as cle:
                logger.warning(f"[图片清理] 删除临时图片失败: {img_p}, 错误: {cle}")
...
def send_reply(user_id, sender_name, username, original_merged_message, reply, is_system_message=False):
    ...
    try:
        ...
        wx_op_lock.acquire()  # 发送期间锁定微信窗口操作，防止后台线程抢焦点
        ...
        # 记录"正在打字发送"的文本片段，供剧场"被打断草稿"机制使用
        with _typing_drafts_lock:
            _typing_drafts[user_id] = [c for (t, c) in message_actions if t == 'text']
            ...
            if action_type == 'emoji':
                ...
                for attempt in range(3):
                    try:
                        ...
                        record_sent_text(content)  # 记录即将发送的文本，用于自消息识别
                        send_result = _send_msg(user_id, content)
        ...
        is_sending_message = False
        # 发送结束后清理"打字草稿"记录（若仍有未发完的，留给剧场作为被打断草稿）
        with _typing_drafts_lock:
            d = _typing_drafts.get(user_id)
            if d and not d:
                _typing_drafts.pop(user_id, None)
        try:
            wx_op_lock.release()
        except Exception:
            pass
...
def clean_up_temp_files ():
    ...
    # 清理本地监听(WeFlow)下载的媒体目录，防止图片/视频等临时文件无限堆积占满磁盘
    try:
        localdb_media_dir = os.path.join(root_dir, MEMORY_TEMP_DIR, 'localdb_media')
        if os.path.isdir(localdb_media_dir):
            shutil.rmtree(localdb_media_dir, ignore_errors=True)
            logger.info(f"目录 {localdb_media_dir} 已成功删除")
    except Exception as e:
        logger.error(f"删除本地监听媒体目录失败: {str(e)}")
...
def save_recurring_reminders():
    ...
    with recurring_reminder_lock: # 获取锁保证线程安全
        ...
        # 唯一 tmp 名 + os.replace 原子替换（Windows 上 shutil.move 覆盖已存在文件非原子）
        temp_file_path = f"{RECURRING_REMINDERS_FILE}.{os.getpid()}.{uuid.uuid4().hex}.tmp"
        try:
            with open(temp_file_path, 'w', encoding='utf-8') as f:
                json.dump(reminders_to_save, f, ensure_ascii=False, indent=4)
                f.flush()
                os.fsync(f.fileno())
            os.replace(temp_file_path, RECURRING_REMINDERS_FILE)
...
def _parse_reminder_sched(reminder, now_dt):
    """返回该提醒"本次/今天"的计划触发 datetime；无法解析返回 None。

    recurring: 用 time_str('HH:MM') 与今天日期组合；
    one-off:   直接解析 target_datetime_str('YYYY-MM-DD HH:MM')。
    """
    rtype = reminder.get('reminder_type')
    if rtype == 'recurring':
        ts = str(reminder.get('time_str') or '').strip()
        try:
            hh, mm = ts.split(':')[:2]
            return datetime.combine(now_dt.date(), dt.time(int(hh), int(mm)))
        except Exception:
            return None
    elif rtype == 'one-off':
        s = str(reminder.get('target_datetime_str') or '').strip()
...
def _deliver_reminder(user_id, content, reminder_type, now_dt):
    """把提醒投递到用户消息队列（走联网/AI 流程），并按配置发起语音通话。"""
    logger.info(f"正在为用户 {user_id} 触发【{reminder_type}】提醒：{content}")
    try:
        prefix = f"每日提醒：{content}" if reminder_type == 'recurring' else f"一次性提醒：{content}"
        formatted_message = f"[{now_dt.strftime('%Y-%m-%d %A %H:%M:%S')}] {prefix}"
        with queue_lock:
            if user_id not in user_queues:
                user_queues[user_id] = {
                    'messages': [formatted_message],
                ...
                user_queues[user_id]['messages'].append(formatted_message)
                user_queues[user_id]['last_message_time'] = time.time()
        logger.info(f"已将{reminder_type}提醒 '{content}' 添加到用户 {user_id} 的消息队列")
        ...
        logger.error(f"投递提醒失败，用户 {user_id}，提醒：{content}：{e}")


def recurring_reminder_checker():
    """后台线程：基于"到期时间比较"触发重复/长期一次性提醒。

    旧实现按精确分钟串相等匹配 + sleep(58)，一旦因漂移/耗时跳过某分钟，该提醒就永不触发；
    且一次性提醒按"早先副本的索引"删除，列表变动时会删错条目。
    新实现：now >= 计划时间即触发（带最大迟到上限，避免停机后洪泛或荒谬地补发），
    每条提醒记录 last_fired_date / done 状态，按身份(done 标记)删除，杜绝索引错位。
    """
    MAX_LATE_SECONDS = 300  # 超过此迟到视为错过：标记状态但不补发
    ...
    while True:
        try:
            ...
            with recurring_reminder_lock:
                snapshot = list(recurring_reminders)  # 引用副本，元素仍是同一批 dict

            quiet_blocked = is_quiet_time() and not ALLOW_REMINDERS_IN_QUIET_TIME
            ...
            for reminder in snapshot:
                ...
                sched_dt = _parse_reminder_sched(reminder, now_dt)
                if sched_dt is None:
                    continue
                sched_ts = sched_dt.timestamp()

                if rtype == 'recurring':
                    if reminder.get('last_fired_date') == today_str:
                        continue  # 今天已触发/已处理
                    if now_ts < sched_ts:
                        continue  # 未到时间
                    if now_ts - sched_ts > MAX_LATE_SECONDS:
                        reminder['last_fired_date'] = today_str  # 错过太久（如停机），今天不补发
                        state_changed = True
                        logger.info(f"每日提醒迟到超过{MAX_LATE_SECONDS}s，跳过补发: 用户 {user_id}, 计划 {sched_dt.strftime('%H:%M')}")
                        continue
                    if quiet_blocked:
                        reminder['last_fired_date'] = today_str
                        state_changed = True
                        logger.info(f"安静时间抑制每日提醒: 用户 {user_id}, 内容: {content}")
                        continue
                    _deliver_reminder(user_id, content, 'recurring', now_dt)
                    reminder['last_fired_date'] = today_str
                    state_changed = True
                    ...
                    if now_ts - sched_ts > MAX_LATE_SECONDS:
                        reminder['done'] = True
                        state_changed = True
                        logger.info(f"一次性提醒迟到超过{MAX_LATE_SECONDS}s，标记完成不补发: 用户 {user_id}, 计划 {reminder.get('target_datetime_str')}")
                        continue
                    if quiet_blocked:
                        reminder['done'] = True
                        state_changed = True
                        logger.info(f"安静时间抑制一次性提醒(将不再触发): 用户 {user_id}, 内容: {content}")
                        continue
                    _deliver_reminder(user_id, content, 'one-off', now_dt)
                    reminder['done'] = True
                    state_changed = True

            # 删除已完成的一次性提醒（按 done 标记，避免索引错位）；状态变化时持久化
            with recurring_reminder_lock:
                before = len(recurring_reminders)
                recurring_reminders[:] = [
                    r for r in recurring_reminders
                    if not (isinstance(r, dict) and r.get('reminder_type') == 'one-off' and r.get('done'))
                ]
                removed = before - len(recurring_reminders)
                if removed:
                    logger.info(f"已删除 {removed} 条已完成的一次性提醒。")
                if removed or state_changed:
                    save_recurring_reminders()
...
# 消息轰炸运行时开关：网页勾选 ENABLE_BOMBING 是功能总开关；运行时发 /轰炸 才开始轰炸，/轰炸停 停止
BOMBING_ENABLED = False


def send_bombing_once():
    """发送一次轰炸消息"""
    try:
        if not BOMBING_USER:
            return
        wx_op_lock.acquire()  # 发送期间禁止其他窗口操作
        try:
            sent_any = False
            if BOMBING_FILE and os.path.exists(BOMBING_FILE):
                _send_file(BOMBING_USER, BOMBING_FILE)
                logger.info(f"[轰炸] 发送文件给 {BOMBING_USER}: {os.path.basename(BOMBING_FILE)}")
                sent_any = True
            if BOMBING_CONTENT:
                record_sent_text(BOMBING_CONTENT)
                _send_msg(BOMBING_USER, BOMBING_CONTENT)
                logger.info(f"[轰炸] 发送消息给 {BOMBING_USER}: {BOMBING_CONTENT[:30]}")
                sent_any = True
            if not sent_any:
                logger.info("[轰炸] 没有可发送的内容（请填写消息内容或文件路径）")
        finally:
            wx_op_lock.release()
    except Exception as e:
        logger.error(f"[轰炸] 发送失败: {e}")


def bombing_worker():
    """消息轰炸后台线程"""
    global BOMBING_ENABLED
    last_send_time = 0
    while True:
        try:
            if ENABLE_BOMBING and BOMBING_ENABLED:
                try:
                    interval = float(BOMBING_INTERVAL)
                except Exception:
                    interval = 5.0
                interval = max(0.1, min(interval, 6000))
                if time.time() - last_send_time >= interval:
                    send_bombing_once()
                    last_send_time = time.time()
        except Exception as e:
            logger.error(f"[轰炸] 线程错误: {e}")
        time.sleep(0.2)
...
def main():
    try:
        ...
        # --- 初始化本地数据监听 (UserDataIsSafeFromUsers HTTP API)，取代 wxauto 窗口监听 ---
        global localdb_listener
        for user_name in user_names:
            if user_name == ROBOT_WX_NAME:
                logger.error(f"\033[31m您填写的用户列表中包含自己登录的微信昵称，请删除后再试！\033[0m")
                exit(1)
        localdb_listener = LocalDbListener(
            api_base=LOCALDB_API_BASE,
            token=LOCALDB_API_TOKEN,
            listen_names=user_names,
            poll_interval=LOCALDB_POLL_INTERVAL,
            media_dir=os.path.join(root_dir, MEMORY_TEMP_DIR, 'localdb_media'),
            only_recent_seconds=LOCALDB_ONLY_RECENT_SECONDS,
            on_message=lambda who, msg: message_listener(msg, LocalDbChat(who)),
            on_sessions=update_group_chat_cache_from_sessions,
        )
        global localdb_listener_instance
        localdb_listener_instance = localdb_listener
        targets = localdb_listener.resolve_target_sessions()
        if not targets:
            logger.error("\033[31m本地数据监听初始化失败：未能在本地库中找到任何配置的会话。\033[0m")
            logger.error("请确认：1) WeFlow 已启动并在「设置 → HTTP API」中开启（端口 5031）；2) LISTEN_LIST 中的昵称与 WeFlow 会话一致；3) 若 WeFlow 设置了访问令牌，请同步填入 LOCALDB_API_TOKEN。")
            logger.warning("监听线程仍会持续重试，待 WeFlow HTTP API 就绪后自动恢复，无需重启 bot。")
        localdb_listener.start()
        logger.info("本地数据监听线程已启动（UserDataIsSafeFromUsers HTTP API）")
        ...
        # 启动消息轰炸线程
        bombing_thread = threading.Thread(target=bombing_worker, name="BombingWorker")
        bombing_thread.daemon = True
        bombing_thread.start()
        logger.info("消息轰炸线程已启动。")
