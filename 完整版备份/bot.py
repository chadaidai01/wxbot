...
def _theater_handle(user, message, image_paths=None, drafts=None, reply_probability=1.0, force_reply=False):
...
def message_listener(msg, chat):
    ...
    if not is_group_chat: 
        ...
        # 剧场/全局阅读模式：开启后该群消息全局读取，不再依赖 @/关键词/概率
        if is_theater_chat(who):
            should_process_this_message = True
            logger.info(f"群聊 '{who}' 处于剧场/全局阅读模式，消息全局读取并进入叙事。")
