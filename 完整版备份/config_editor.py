...
def validate_config_types(config_path):
    ...
    try:
        ...
        int_fields = ['MAX_GROUPS', 'MAX_TOKEN', 'QUEUE_WAITING_TIME', 'EMOJI_SENDING_PROBABILITY', 
                     ...
                     'THEATER_GROUP_REPLY_PROBABILITY']
...
def api_save_theater(name):
    ...
    if 'overlays_text' in payload:
        ...
        for ln in _split_lines(payload.get('overlays_text')):
            ...
            ov.append({'layer': layer, 'target': target, 'description': desc, 'ts': time.time()})
        data['overlays'] = ov[:30]
    ...
    if 'clear_done_intents' in payload and payload.get('clear_done_intents'):
    ...
    return jsonify({'success': True, 'message': f'剧场《{name}》记忆已保存'})
