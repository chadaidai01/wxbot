...
def hide_api_key(api_key):
    """
    隐藏API Key/Token，只显示前4位和后4位，中间用*替代。
    短密钥（<=8位）此前会原样返回造成明文泄露，现统一打码。
    """
    if not api_key:
        return api_key
    s = str(api_key)
...
# 需要在网页显示时打码、保存时识别掩码并保留原值的敏感字段（统一常量，供多处复用）
_API_KEY_FIELDS = ['DEEPSEEK_API_KEY', 'MOONSHOT_API_KEY', 'ONLINE_API_KEY',
                   'ASSISTANT_API_KEY', 'FORUM_API_KEY', 'LOCALDB_API_TOKEN']
...
def validate_config_types(config_path):
    ...
    try:
        ...
        int_fields = ['MAX_GROUPS', 'MAX_TOKEN', 'QUEUE_WAITING_TIME', 'EMOJI_SENDING_PROBABILITY', 
                     ...
                     'THEATER_ADVANCE_INTERVAL_MINUTES', 'THEATER_PROACTIVE_THRESHOLD', 'THEATER_SCRIPT_BUDGET',
                     'THEATER_GROUP_REPLY_PROBABILITY', 'THEATER_RECENT_CONTEXT', 'THEATER_RECENT_LOOKBACK',
                     'THEATER_BUBBLE_INTERVAL_HOURS', 'THEATER_ALTER_BASE_THRESHOLD',
                     'THEATER_MIN_PROACTIVE_INTERVAL_MINUTES', 'THEATER_CONTINUITY_REFRESH_TURNS',
                     'THEATER_FOLLOWUP_MINUTES_1', 'THEATER_FOLLOWUP_MINUTES_2',
                     'THEATER_PREPLAN_REVIEW_HOUR', 'THEATER_GROUP_WILLINGNESS_HALF_LIFE_MINUTES']
        
        # 检查应该是浮点数但被保存为字符串的配置项
        float_fields = ['TEMPERATURE', 'MOONSHOT_TEMPERATURE', 'MIN_COUNTDOWN_HOURS', 'MAX_COUNTDOWN_HOURS',
                       'AVERAGE_TYPING_SPEED', 'RANDOM_TYPING_SPEED_MIN', 'RANDOM_TYPING_SPEED_MAX', 'TYPING_DELAY_MIN', 'TYPING_DELAY_MAX',
                       'ONLINE_API_TEMPERATURE', 'RESTART_INTERVAL_HOURS', 'ASSISTANT_TEMPERATURE',
                       'LOCALDB_POLL_INTERVAL', 'LOCALDB_ONLY_RECENT_SECONDS', 'THEATER_ACTIVE_CONSEQUENCE_DECAY',
                       'THEATER_GROUP_WILLINGNESS_THRESHOLD']
...
# secret_key 持久化：避免每次重启都更换导致所有登录会话失效（多 worker 下也能共享）。
...
# 上传大小上限，防止磁盘/内存 DoS（目录导入可能较大，给一个宽松上限）
...
def login():
    ...
    if request.method == 'POST':
        ...
        # 拒绝空密码：若 LOGIN_PASSWORD 为空，任意空提交都会相等而绕过登录
        if not stored_pwd:
            app.logger.error("已启用登录密码但 LOGIN_PASSWORD 为空，拒绝登录（请设置非空密码）")
            return render_template('login.html', error="密码未配置，请联系管理员")
        # 常量时间比较，避免计时侧信道
        if hmac.compare_digest(str(password), str(stored_pwd)):
...
def start_bot():
    ...
    if bot_process is None or bot_process.poll() is not None:
        ...
        bot_process = subprocess.Popen(
            ...
            cwd=bot_dir  # 固定工作目录，确保 bot.py 的相对路径(chat_contexts.json 等)与本编辑器一致
        )
...
def submit_config():
    ...
    try:
        ...
        # 防御：LISTEN_LIST 条目可能不足 2 个元素（第三项可省略），直接 item[1] 会 IndexError
        old_listen_list_map = {}
        for item in current_config_before_update.get('LISTEN_LIST', []):
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                old_listen_list_map[item[0]] = item[1]

        new_values_for_config_py = {}
        
        # 处理API Key字段的特殊逻辑
        api_key_fields = list(_API_KEY_FIELDS)
        ...
        theater_flags_from_form = request.form.getlist('theater_flag')  # 每行: '0'=关 '1'=开（隐藏0+复选1）

        processed_listen_list = []
        listen_list_provided = bool(nicknames_from_form and prompt_files_from_form
                                    and len(nicknames_from_form) == len(prompt_files_from_form))
        if listen_list_provided:
            for i, (nick, pf) in enumerate(zip(nicknames_from_form, prompt_files_from_form)):
                ...
                if nick_stripped and pf_stripped: 
                    flag = '1' if (i < len(theater_flags_from_form) and theater_flags_from_form[i] == '1') else '0'
                    processed_listen_list.append([nick_stripped, pf_stripped, flag == '1'])
        # 仅当表单确实提供了合法的用户列表时才覆盖；数量不匹配/为空时保留原值，避免误清空 LISTEN_LIST
        if listen_list_provided:
            new_values_for_config_py['LISTEN_LIST'] = processed_listen_list
            new_listen_list_map = {item[0]: item[1] for item in processed_listen_list if len(item) >= 2}
        else:
            new_listen_list_map = dict(old_listen_list_map)
        ...
        boolean_fields = [
            ...
            'ENABLE_TEXT_COMMANDS', 'ENABLE_AUTO_PAT_BACK', 'ENABLE_BOMBING',
            'ENABLE_ROLLPIG', 'ENABLE_STARRAIL', 'ENABLE_IMAGE_USE_CHAT_API', 'ENABLE_IMAGE_NATIVE_MULTIMODAL',
            'ENABLE_THEATER', 'THEATER_RESPECT_QUIET_TIME', 'THEATER_ALTER_ENABLED',
            'THEATER_TIME_DIRECTOR_ENABLED', 'THEATER_PREPLAN_ENABLED', 'THEATER_PREPLAN_ANCHOR_ADVANCE',
            'THEATER_PROACTIVE_ENABLED', 'THEATER_COMPACT_ENABLED', 'THEATER_GROUP_WILLINGNESS_ENABLED', 'THEATER_STICKER_STEAL_ENABLED', 'THEATER_STICKERS_ENABLED'
        ]
        ...
        for key_from_form in request.form:
            ...
            if key_from_form == 'REPEATER_LIST':
                # 每行格式: 群名|某人
                repeater_list = []
                if value_from_form:
                    for line in value_from_form.replace('，', '|').splitlines():
                        line = line.strip()
                        if not line:
                            continue
                        parts = line.split('|')
                        if len(parts) >= 2 and parts[0].strip() and parts[1].strip():
                            repeater_list.append({'group': parts[0].strip(), 'target': parts[1].strip()})
                new_values_for_config_py[key_from_form] = repeater_list
                continue

            if key_from_form == 'GROUP_PROACTIVE_REPLY_PROBABILITY':
                try:
                    new_values_for_config_py[key_from_form] = int(value_from_form) if value_from_form.strip().isdigit() else 0
                except Exception:
            ...
            if key_from_form in current_config_before_update:
                ...
                elif key_from_form in ["MIN_COUNTDOWN_HOURS", "MAX_COUNTDOWN_HOURS", "AVERAGE_TYPING_SPEED", "RANDOM_TYPING_SPEED_MIN", "RANDOM_TYPING_SPEED_MAX", "TYPING_DELAY_MIN", "TYPING_DELAY_MAX", "TEMPERATURE", "MOONSHOT_TEMPERATURE", "ONLINE_API_TEMPERATURE", "ASSISTANT_TEMPERATURE", "RESTART_INTERVAL_HOURS", "FORUM_TEMPERATURE"]: 
                    try:
                        # 先确保值是字符串类型，然后进行转换
...
def update_config(new_values):
    ...
    with FileLock(lock_path):
        ...
        try:
            ...
            # 写前语法校验：若按行改写后产生非法 Python（例如多行列表被拆坏），
            # 直接放弃写入并报错，绝不破坏原 config.py（避免 bot 再也无法 import config）。
            try:
                compile(new_content, config_path, 'exec')
            except SyntaxError as se:
                raise Exception(f"配置改写后语法校验失败，已放弃写入以保护原文件: 第{se.lineno}行 {se.msg}")

            # 写入临时文件并 fsync，成功后用 os.replace 原子替换（Windows 上 shutil.move 非原子）
            fd, temp_file_name = tempfile.mkstemp(dir=script_dir, suffix='.tmp', prefix='config_')
            with os.fdopen(fd, 'w', encoding='utf-8') as temp_file:
                temp_file.write(new_content)
                temp_file.flush()
                os.fsync(temp_file.fileno())
            os.replace(temp_file_name, config_path)
            temp_file_name = None  # 已成功替换，无需清理
        except Exception as e:
            # 捕获并记录异常，以便排查问题
            raise Exception(f"更新配置文件失败: {e}")
        finally:
            # 失败路径清理临时文件，避免在项目目录留下垃圾
...
def index():
    ...
    if request.method == 'POST':
        try:
            ...
            theater_flags = request.form.getlist('theater_flag')  # 每行: '0'/'1'
            _listen_rows = []
            for i, (nick, pf) in enumerate(zip(nicknames, prompt_files)):
                if not (nick.strip() and pf.strip()):
                    continue
                flag = '1' if (i < len(theater_flags) and theater_flags[i] == '1') else '0'
                _listen_rows.append([nick.strip(), pf.strip(), flag == '1'])
            new_values['LISTEN_LIST'] = _listen_rows
            ...
            for var in submitted_fields:
                ...
                # 保护 API Key/Token：若提交的是打码后的隐藏值，保留原值，
                # 避免把 "sk-2****" 这类掩码串写回 config.py 永久毁掉真实密钥（修复 index POST 路径）。
                if var in _API_KEY_FIELDS and is_hidden_api_key(value_from_form):
                    new_values[var] = original_value if original_value is not None else ''
                    continue
    ...
    try:
        ...
        try:
            ...
            for relative_path, file_data in files_dict.items():
                ...
                # 安全：逐段净化，拒绝 .. / 绝对路径 / 盘符，防止路径穿越写任意文件（RCE）。
                # 保留中文/字母/数字/下划线/点/连字符，其余字符（含 : / \）替换为 _。
                safe_parts = []
                rejected = False
                for seg in relative_path.split('/'):
                    if seg in ('', '.', '..'):
                        rejected = True
                        break
                    seg_clean = re.sub(r'[^\w.\u4e00-\u9fff-]', '_', seg)
                    if not seg_clean or seg_clean in ('.', '..'):
                        rejected = True
                        break
                    safe_parts.append(seg_clean)
                if rejected or not safe_parts:
                    app.logger.warning(f"导入：跳过不安全的文件路径: {relative_path}")
                    continue

                # 创建完整的文件路径
                full_path = os.path.join(temp_dir, *safe_parts)

                # 二次校验：最终真实路径必须仍位于 temp_dir 内，否则拒绝
                if not os.path.realpath(full_path).startswith(temp_dir_real + os.sep):
                    app.logger.warning(f"导入：跳过越界文件路径: {relative_path} -> {full_path}")
                    continue

                # 确保目录存在
                dir_path = os.path.dirname(full_path)
                if dir_path and os.path.realpath(dir_path) != temp_dir_real:
...
def save_user_chat_context(username):
    ...
    # poll() is None 才表示进程"正在运行"；旧写法 is not None 逻辑取反，
    # 导致 bot 运行时反而放行并发写、bot 已停止时却报"正在运行"。
    if bot_process and bot_process.poll() is None:
    ...
    with FileLock(CHAT_CONTEXTS_LOCK_FILE):
        try:
            ...
            # 唯一 tmp 名 + os.replace 原子替换：避免与 bot 进程共用 chat_contexts.json.tmp 互相截断，
            # 也避免 Windows 上 shutil.move 覆盖已存在文件时的非原子写入。
            temp_file_path = f"{CHAT_CONTEXTS_FILE}.{os.getpid()}.{uuid.uuid4().hex}.tmp"
            try:
                with open(temp_file_path, 'w', encoding='utf-8') as f:
                    json.dump(all_contexts, f, ensure_ascii=False, indent=4)
                    f.flush()
                    os.fsync(f.fileno())
                os.replace(temp_file_path, CHAT_CONTEXTS_FILE)
            finally:
...
def test_forum_ai(character_name):
    ...
    try:
        ...
        return f"""
        <h2>AI判断测试结果</h2>
        <p><strong>角色:</strong> {escape(character_name)}</p>
        <p><strong>判断结果:</strong> {'发布' if should_post else '不发布'}</p>
        <p><strong>生成内容:</strong> {escape(content)}</p>
        <p><strong>测试时间:</strong> {escape(result['timestamp'])}</p>
        <hr>
        <p><a href="javascript:history.back()">返回</a> | <a href="/forum/{escape(character_name)}" target="_blank">查看论坛</a></p>
        """
        
    except Exception as e:
        app.logger.error(f"测试AI判断失败: {e}")
        return f"测试失败: {escape(str(e))}", 500
...
def get_default_config():
    return {
        ...
        "STICKER_CLASSIFY_API_KEY": '',
        "STICKER_CLASSIFY_BASE_URL": '',
        "STICKER_CLASSIFY_MODEL": '',
        "MOONSHOT_TEMPERATURE": 0.8,
        "ENABLE_IMAGE_USE_CHAT_API": False,
        "ENABLE_IMAGE_NATIVE_MULTIMODAL": False,
        ...
        "GROUP_PROACTIVE_REPLY_CONTEXT_COUNT": 30,
        ...
        "ENABLE_THEATER": True,
        "THEATER_DIR": 'Theater',
        "THEATER_ADVANCE_INTERVAL_MINUTES": 20,
        "THEATER_PROACTIVE_THRESHOLD": 60,
        "THEATER_RESPECT_QUIET_TIME": True,
        "THEATER_SCRIPT_BUDGET": 40,
        "THEATER_RECENT_CONTEXT": 12,
        ...
        "THEATER_GROUP_COOLDOWN_SECONDS": 30,
        "THEATER_STICKER_STEAL_ENABLED": True,
        "THEATER_STICKERS_ENABLED": True,
        "THEATER_STICKER_DIR": 'emojis',
        "STICKER_STEAL_MEMORY_FILE": 'Memory_Temp/sticker_steal_memory.json'
    }
...
def kill_process_using_port(port):
    ...
    """
    # PORT 可能是字符串，统一转 int，否则 conn.laddr.port == port 永不匹配（静默失效）
    try:
        port = int(port)
    except (TypeError, ValueError):
        print(f"端口值非法（{port!r}），跳过端口占用清理。")
        return
    current_pid = os.getpid()
    try:
        connections = psutil.net_connections()
    except Exception as e:
        print(f"枚举网络连接失败，跳过端口占用清理: {e}")
        return
    # 遍历所有连接
    for conn in connections:
        try:
            ...
            if conn.status not in ('LISTEN', 'LISTENING'):
                continue
            # 关键：Windows 上无主连接 conn.pid 为 None，psutil.Process(None) 会返回"当前进程"，
            # 直接 kill 会导致编辑器自杀（表现为 Run.bat 一闪即退）。这里跳过 None 与自身进程。
            if conn.pid is None or conn.pid == current_pid:
                continue
            ...
            print(f"结束占用端口 {port} 的进程时出现异常：{e}")
...
if __name__ == '__main__':
    ...
    if config.get('ENABLE_LOGIN_PASSWORD', False):
        print("")
        # 不在控制台明文回显登录密码，避免被屏幕共享/日志泄露
        print("\033[33m🔐 登录密码: 已设置（见 config.py 的 LOGIN_PASSWORD，或环境变量 WXBOT_LOGIN_PASSWORD）\033[0m")
