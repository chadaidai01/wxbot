...
def merge_context(context_list):
    ...
    list 内容（含图片）在合并时保留结构，不转纯文本，保证图片历史可回放。
    """
    if not context_list:
        return []
    merged = []
    ...
    for item in context_list:
        ...
        if role == last_role:
            ...
            if buffer:
                merged.append({'role': last_role, 'content': _merge_same_role_contents(buffer)})
            buffer = [content]
            last_role = role
    if buffer:
        merged.append({'role': last_role, 'content': _merge_same_role_contents(buffer)})
...
def _merge_same_role_contents(buffer):
    """合并连续同 role 的 content：全部是字符串则 join；含 list 则拼接为多模态 list。"""
    has_list = any(isinstance(c, list) for c in buffer)
    if not has_list:
        return '\n'.join(str(c) for c in buffer if c is not None)
    # 含图片结构体：把文本和图片都收集进多模态 content 数组
    parts = []
    for c in buffer:
        if isinstance(c, str):
            if c.strip():
...
def get_deepseek_response(message, user_id, store_context=True, is_summary=False, image_paths=None):
    ...
    参数:
        ...
        image_paths (list, optional): 图片/表情本地路径列表。非空时构造多模态消息，
                              图片以 data URL 直接发给 Chat 模型原生识别。
    """
    try:
        ...
        if image_paths:
            _diag_write("get_response", f"user={user_id} 收到{len(image_paths)}个路径 有效{len(valid_image_paths)}个 无效{[p for p in image_paths if p not in valid_image_paths]}")
        ...
        if has_images:
            ...
            if text_for_model:
                current_user_content.append({"type": "text", "text": text_for_model})
...
def _convert_local_images_to_data_urls(messages_to_send):
    """把消息列表中的本地图片路径（_local_path 标记）转换为 data URL。

    兼容 content 为 str 或 list（OpenAI 多模态格式）。文件缺失时回退为文本 "[图片]"。
    """
    ...
    for msg in messages_to_send:
        ...
        new_content = _content_to_multimodal(content)
...
def _content_to_multimodal(content):
    """将单条消息的 content 处理为可用于 API 的多模态格式。

    - content 为 str 且不含图片：原样返回
    - content 为 list（多模态结构）：把含 _local_path 的 image_url 转换为 data URL
    """
    if isinstance(content, str):
        # 纯文本，不需要处理（注意：历史里若存过文字形式的图片占位则保留）
        return content
...
def _diag_write(tag, text):
    """运行时诊断：写入 bot 目录下的 bot_diag.log，便于排查图片识别链路问题。"""
    try:
        diag_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bot_diag.log')
...
def _wechat_client_origin():
    """返回微信主窗口左上角的屏幕坐标 (x, y)。
    BoundingRectangle 是相对窗口（含边框/标题栏）的坐标，加上窗口左上角即得屏幕坐标。
    返回 (origin_x, origin_y)，找不到返回 None。
    """
    try:
        ...
        for hwnd in window_list:
            ...
            if '微信' in title or 'Weixin' in title or 'WeChat' in title:
                ...
                wr = _R()
                user32.GetWindowRect(hwnd, _ct.byref(wr))
                return (wr.l, wr.t)
        return None
    except Exception:
        return None
...
def _focus_wechat_window():
    """把可见的微信主窗口带到前台并激活。库的 UI 操作依赖窗口在前台。"""
    try:
        import ctypes as _ct
        user32 = _ct.windll.user32
        window_list = []
...
def call_chat_api_with_retry(messages_to_send, user_id, max_retries=2, is_summary=False):
    ...
    返回:
        str: API 返回的文本回复。
    """
    # 发送前把消息里的本地图片路径转换为 data URL（Chat 模型原生多模态识别）
...
def keep_alive():
    ...
    missing_since = {}  # user -> 首次检测到缺失的时间戳，用于退避，避免反复重连回灌历史消息
    ...
    while True:
        try:
            ...
            # 清理已恢复监听的用户
            for user in list(missing_since.keys()):
                if user not in missing_users:
                    missing_since.pop(user, None)

            if missing_users:
                logger.warning(f"检测到 {len(missing_users)} 个用户从监听列表中丢失: {', '.join(missing_users)}")
                for user in missing_users:
                    if user not in missing_since:
                        missing_since[user] = time.time()
                        continue
                    if time.time() - missing_since[user] < 15:
                        # 瞬时抖动/短暂刷新导致的丢失不立即重连，避免重复AddListenChat回灌历史消息
                        logger.info(f"用户 '{user}' 持续缺失不足15秒，暂不重连，避免回灌历史消息。")
                        continue
                    try:
                        logger.info(f"正在尝试重新添加用户 '{user}' 到监听列表...")
                        # 使用与程序启动时相同的回调函数 `message_listener` 重新添加监听
                        wx_op_lock.acquire()  # 发送期间禁止重连抢焦点
                        try:
...
MSG_MAX_AGE_SECONDS = 300        # 超过该时长的消息视为历史消息，不处理（防止回复很久以前的消息）
MSG_DEDUP_WINDOW_SECONDS = 60    # 该时间窗内出现过的相同消息视为重复投递，不处理
MSG_DEDUP_CACHE_SIZE = 500       # 去重缓存上限

_seen_msg_cache = {}
_seen_msg_lock = threading.Lock()

def _get_msg_age_seconds(msg):
    """获取消息年龄（秒）。取不到时间字段时返回 None。"""
...
def _message_is_replay(who, sender, msgtype, content, msg):
    """判断消息是否为历史消息回灌或重复投递，是则返回 True（应忽略）。"""
    global _seen_msg_cache
    age = _get_msg_age_seconds(msg)
    if age is not None and age > MSG_MAX_AGE_SECONDS:
        logger.info(f"[消息防重] 忽略 {who}/{sender} 的 {age:.0f} 秒前历史消息，防止回复旧消息。")
        return True
    key = (who, sender, msgtype, content)
    now = time.time()
    with _seen_msg_lock:
        prev = _seen_msg_cache.get(key)
        if prev is not None and (now - prev) < MSG_DEDUP_WINDOW_SECONDS:
            logger.info(f"[消息防重] 忽略 {who}/{sender} 的重复消息，防止旧消息重复回复。")
            return True
        _seen_msg_cache[key] = now
        if len(_seen_msg_cache) > MSG_DEDUP_CACHE_SIZE:
            try:
                _seen_msg_cache.pop(next(iter(_seen_msg_cache)))
            except Exception:
                pass
    return False
...
def message_listener(msg, chat):
    ...
    # --- 引用图片识别（群聊/私聊通用） ---
    # 该 wxauto 版本会把「引用」消息渲染成文本，例如：
    #   "这是谁 引用 Cheshako 的消息 : 图片"
    #   "引用 <XX> 消息 : 图片"
    # 此时 msg.type 是 'text'，被引用图片的路径拿不到，只能用截图兜底识别。
    if msgtype == 'text' and ENABLE_IMAGE_RECOGNITION and original_content and \
            ('引用' in original_content) and ('图片' in original_content or '照片' in original_content):
        try:
            # 判断被引用的到底是图片还是纯文字：库渲染格式里 ": 图片" / ": 动画表情" 表示引用内容类型
            import re as _re
            is_quoted_image = bool(_re.search(r'[:：]\s*(图片|照片)\s*$', original_content)) or \
                              ('引用' in original_content and '图片' in original_content and
                               '表情' not in original_content.split('引用')[-1][:40])
            if is_quoted_image:
                logger.info(f"[引用图片识别] 检测到引用图片消息 (来自 {sender})，尝试下载被引用原图...")
                _diag_write("引用检测", f"type={msgtype} sender={sender} 内容={original_content[:100]}")
                capture_path = None
                # 从聊天记录找到被引用的原图消息，滚动到可见后下载原图，不截图
                try:
                    # 库的 UI 操作依赖微信窗口在前台，先聚焦微信窗口
                    _focus_wechat_window()
                    ref_chat = chat
                    msgs = None
                    for g_attempt in range(3):
                        try:
                            msgs = ref_chat.GetAllMessage()
                            if msgs is not None:
                                break
                        except Exception as ge:
                            _diag_write("引用原图", f"GetAllMessage 失败 (第{g_attempt+1}次): {ge}")
                            _focus_wechat_window()
                            time.sleep(1.0)
                    if not msgs:
                        _diag_write("引用原图", "GetAllMessage 返回空")
                    else:
                        try:
                            type_counts = {}
                            for fm in msgs:
                                t = getattr(fm, 'type', '') or ''
                                type_counts[t] = type_counts.get(t, 0) + 1
                            _diag_write("引用原图", f"聊天记录消息类型统计: {type_counts} 总数={len(msgs)}")
                        except Exception as tc:
                            _diag_write("引用原图", f"类型统计失败: {tc}")
                        # 从最新消息往前找，跳过当前引用消息，找第一条图片消息
                        for fm in reversed(msgs):
                            if fm is msg:
                                continue
                            fm_type = getattr(fm, 'type', '') or ''
                            if fm_type == 'image' and getattr(fm, 'attr', '') == 'friend':
                                # 诊断：记录图片消息控件坐标 + 微信窗口屏幕位置，验证坐标换算
                                try:
                                    img_ctrl2 = getattr(fm, 'control', None)
                                    if img_ctrl2 is not None:
                                        br2 = getattr(img_ctrl2, 'BoundingRectangle', None)
                                        if br2 is not None:
                                            _diag_write("引用原图", f"图片控件Rect=({br2.left},{br2.top},{br2.right},{br2.bottom})")
                                except Exception as ie:
                                    _diag_write("引用原图", f"控件坐标记录失败: {ie}")
                                try:
                                    import ctypes as _ctw
                                    user32w = _ctw.windll.user32
                                    class _RW(_ctw.Structure):
                                        _fields_ = [('l', _ctw.c_long), ('t', _ctw.c_long), ('r', _ctw.c_long), ('b', _ctw.c_long)]
                                    wlist = []
                                    @_ctw.WINFUNCTYPE(_ctw.c_bool, _ctw.c_void_p, _ctw.c_void_p)
                                    def _cbw(h, l):
                                        wlist.append(h)
                                        return True
                                    user32w.EnumWindows(_cbw, 0)
                                    for h in wlist:
                                        if not user32w.IsWindowVisible(h):
                                            continue
                                        nn = user32w.GetWindowTextLengthW(h)
                                        if nn <= 0 or nn >= 80:
                                            continue
                                        bb = _ctw.create_unicode_buffer(nn + 1)
                                        user32w.GetWindowTextW(h, bb, nn + 1)
                                        if '微信' in bb.value or 'Weixin' in bb.value:
                                            rr = _RW()
                                            user32w.GetWindowRect(h, _ctw.byref(rr))
                                            _diag_write("引用原图", f"微信窗口屏幕Rect=({rr.l},{rr.t},{rr.r},{rr.b})")
                                            break
                                except Exception as we:
                                    _diag_write("引用原图", f"窗口坐标记录失败: {we}")
                                # 先滚动到图片消息可见，再下载（下载依赖控件存在）
                                try:
                                    if hasattr(fm, 'roll_into_view'):
                                        fm.roll_into_view()
                                        _diag_write("引用原图", "已滚动到图片消息可见")
                                        time.sleep(0.8)
                                except Exception as rve:
                                    _diag_write("引用原图", f"滚动到可见失败: {rve}")
                                # 多次重试下载
                                for attempt in range(3):
                                    try:
                                        dl_path = fm.download()
                                        if dl_path and os.path.exists(str(dl_path)):
                                            capture_path = str(dl_path)
                                            _diag_write("引用原图", f"下载到被引用原图: {capture_path} (第{attempt+1}次)")
                                            break
                                        else:
                                            _diag_write("引用原图", f"download 返回空 (第{attempt+1}次)")
                                    except Exception as dle:
                                        _diag_write("引用原图", f"下载失败 (第{attempt+1}次): {dle}")
                                        try:
                                            if hasattr(fm, 'roll_into_view'):
                                                fm.roll_into_view()
                                                time.sleep(0.8)
                                    ...
                                    if capture_path is None:
                                        try:
                                            if hasattr(fm, 'save'):
                                                s_path = fm.save()
                                                if s_path and os.path.exists(str(s_path)):
                                                    capture_path = str(s_path)
                                                    _diag_write("引用原图", f"save() 保存到: {capture_path}")
                                                    break
                                        except Exception as save_e:
                                            _diag_write("引用原图", f"save() 失败: {save_e}")
                                if capture_path:
                                    break
                                # download/save 都失败：用图片控件 Rect + 客户区原点换算成屏幕坐标，精确截图
                                try:
                                    origin = _wechat_client_origin()
                                    img_ctrl3 = getattr(fm, 'control', None)
                                    if origin and img_ctrl3 is not None:
                                        br3 = getattr(img_ctrl3, 'BoundingRectangle', None)
                                        if br3 is not None:
                                            o_x, o_y = origin
                                            sc_left = int(br3.left) + o_x
                                            sc_top = int(br3.top) + o_y
                                            sc_w = int(br3.right) - int(br3.left)
                                            sc_h = int(br3.bottom) - int(br3.top)
                                            # 防止越界
                                            if sc_left < 0: sc_left = 0
                                            if sc_top < 0: sc_top = 0
                                            if sc_w > 0 and sc_h > 0:
                                                import pyautogui as _pag3
                                                img3 = _pag3.screenshot(region=(sc_left, sc_top, sc_w, sc_h))
                                                save_dir3 = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'wxautox文件下载')
                                                os.makedirs(save_dir3, exist_ok=True)
                                                save_path3 = os.path.join(save_dir3, f"quoted_img_{int(time.time()*1000)}.png")
                                                img3.save(save_path3)
                                                capture_path = save_path3
                                                _diag_write("引用原图", f"控件区域精确截图成功: {save_path3} 屏幕=({sc_left},{sc_top},{sc_w},{sc_h}) 客户区原点=({o_x},{o_y})")
                                                break
                                            else:
                                                _diag_write("引用原图", f"控件Rect异常: {sc_w}x{sc_h}")
                                        else:
                                            _diag_write("引用原图", "图片控件无BoundingRectangle")
                                    else:
                                        _diag_write("引用原图", f"无法定位客户区原点: origin={origin}")
                                except Exception as sce2:
                                    _diag_write("引用原图", f"控件精确截图失败: {sce2}")
                except Exception as ge:
                    logger.warning(f"[引用图片识别] 查找原图失败: {ge}")
                    _diag_write("引用原图", f"查找异常: {ge}")
                if capture_path:
                    # 原生多模态：把图片路径暂存到消息对象，由 handle_wxauto_message 收集后交给 Chat 模型
                    try:
                        existing = getattr(msg, '_quoted_image_paths', None) or []
                        existing.append(capture_path)
                        setattr(msg, '_quoted_image_paths', existing)
                        logger.info(f"[引用图片识别] 已暂存被引用图片: {capture_path}")
                    except Exception as se:
                        logger.error(f"[引用图片识别] 暂存图片路径失败: {se}")
                else:
                    logger.warning("[引用图片识别] 下载被引用图片失败，跳过。")
                    _diag_write("引用原图", "下载失败，未获取到图片")
        except Exception as e:
            logger.error(f"[引用图片识别] 处理失败: {e}", exc_info=True)
    ...
    if msgattr == 'tickle':
        ...
        if self_content in ('/轰炸停', '/轰炸停止', '/轰炸关'):
            ...
            return
        # 文本自消息：正常情况已在上方内容匹配处拦截，走到这里说明该文本
        # 不在机器人最近发送记录里，很可能是别人的消息被库误判为 self，
        # 落入下方好友消息流程；非文本（图片/表情等）保守按 self 处理。
        if msgtype == 'text' and not is_self_sent_text(original_content):
            logger.warning(f"[self误判兜底] 消息属性为 self 但内容不匹配机器人最近发送的文本，判定为误判，按好友消息继续处理。")
            # 不 return，落入下方好友消息流程
        else:
    ...
    # 历史消息回灌/重复投递防护：防止监听器重灌旧消息导致回复很久以前的消息
    if _message_is_replay(who, sender, msgtype, original_content, msg):
        return
    ...
    if msgtype == 'quote':
        ...
        # 引用图片识别：若被引用的是图片，尝试提取路径并识别，把结果附加到内容中，
        # 这样群聊/私聊触发回复时，AI 能基于被引用图片的内容作答
        if ENABLE_IMAGE_RECOGNITION:
            quoted_img_path = _extract_quoted_image_path(msg)
            if quoted_img_path:
                try:
                    logger.info(f"[引用图片识别] 提取到被引用图片: {quoted_img_path}")
                    recognized = recognize_image_with_moonshot(quoted_img_path, is_emoji=False)
                    if recognized:
                        original_content = f"{original_content}\n[被引用图片识别结果]: {recognized}"
                        logger.info(f"[引用图片识别] 识别成功: {recognized[:100]}")
                    else:
                        logger.warning("[引用图片识别] 识别返回为空，忽略。")
                except Exception as e:
                    logger.error(f"[引用图片识别] 识别失败: {e}", exc_info=True)
            else:
                logger.debug("[引用图片识别] 未从引用消息中提取到图片路径，跳过。")
                ...
                try:
                    ...
                    try:
                        if msgtype == 'text' and original_content:
                            ...
                            wx.SendMsg(msg=original_content, who=who)
    ...
    if ENABLE_ROLLPIG and msgtype == 'text' and original_content and any(
        ...
        try:
            ...
            try:
                ...
                if img:
                    wx.SendFiles(filepath=str(img), who=who)
            finally:
                wx_op_lock.release()
    ...
    if should_process_this_message:
        ...
        # AI生图：命中触发词时直接生成图片，不走AI聊天
        if ENABLE_IMAGE_GEN and msgtype == 'text':
            prompt = extract_image_prompt(content_for_handler, IMAGE_GEN_TRIGGER)
            if prompt:
                handle_image_generation(msg, who, prompt)
                return
...
def recognize_image_with_moonshot(image_path, is_emoji=False):
    ...
    try:
        ...
        response = requests.post(f"{image_base_url}/chat/completions", headers=headers, json=data)
...
def _extract_quoted_image_path(msg):
    """从引用消息对象中兜底提取被引用图片的本地路径。
    依次尝试多个候选属性，只要值是字符串/Path 且看起来像图片文件（存在或带图片后缀）就返回。
    返回 None 表示未找到可识别的被引用图片。
    """
    if msg is None:
        return None
    candidates = []
    for attr in ('content', 'quote_content', 'quote'):
        try:
    ...
    for c in candidates:
        ...
        low = c.lower()
        # 路径里带图片后缀，或路径本身存在
        if low.endswith(image_suffixes) or os.path.exists(c):
            return c

    # 诊断：未能提取到图片路径时，打印引用消息对象的全部属性与候选值，便于定位图片路径所在属性
    try:
        diag_lines = [f"[引用图片识别-诊断] 未提取到图片路径, 候选值: {candidates}"]
        for a in dir(msg):
            if a.startswith('_'):
                continue
            try:
            ...
            if callable(v):
                continue
            if not isinstance(v, (str, int, float, bool, bytes)) and not isinstance(v, os.PathLike):
                continue
            vs = str(v)
            if len(vs) > 200:
                vs = vs[:200] + '...'
            diag_lines.append(f"attr '{a}' = {type(v).__name__}: {vs}")
        diag_text = "\n".join(diag_lines)
        logger.warning(diag_text)
        try:
            diag_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'quote_diag.log')
            with open(diag_path, 'a', encoding='utf-8') as df:
                df.write(f"\n--- {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ---\n{diag_text}\n")
        except Exception as de:
            logger.error(f"[引用图片识别-诊断] 写入诊断文件失败: {de}")
    except Exception as e:
        logger.error(f"[引用图片识别-诊断] 打印属性失败: {e}")
    return None
...
def handle_wxauto_message(msg, who):
    ...
    try:
        ...
        # 收集图片/表情路径，供 Chat 模型原生多模态识别（不中转文字描述）
        image_paths_for_ai = []
        # 收集 message_listener 中暂存的被引用图片截图路径
        try:
            quoted_paths = getattr(msg, '_quoted_image_paths', None)
            if quoted_paths:
                image_paths_for_ai.extend(quoted_paths)
                logger.info(f"收集到引用图片截图 {len(quoted_paths)} 张: {quoted_paths}")
        except Exception as e:
            logger.error(f"读取引用图片路径失败: {e}")
        ...
        if msg.type in ('image'):
            if ENABLE_IMAGE_RECOGNITION:
                ...
                if img_path:
                    ...
                    logger.info(f"检测到图片消息，准备交给Chat模型原生识别: {img_path}")
                else:
        ...
        # 检查是否为动画表情
            ...
            if ENABLE_EMOJI_RECOGNITION:
                ...
                if img_path:
                    is_emoji = True
                    image_paths_for_ai.append(str(img_path))
                    processed_content = None # 标记为None，稍后被图片内容替换
                    logger.info("检测到动画表情，准备交给Chat模型原生识别...")
                else:
        ...
        # 如果收集到了图片/表情路径：不再中转文字描述，直接让 Chat 模型原生看图。
        # processed_content 记录图片消息标记，图片路径随队列传给 get_deepseek_response。
        if image_paths_for_ai:
            logger.info(f"收集到 {len(image_paths_for_ai)} 张图片/表情，交给 Chat 模型原生识别。")
            _diag_write("handle收集", f"msgtype={msg.type} 图片路径={image_paths_for_ai}")
            if not is_emoji:
                processed_content = (original_content or "[图片]") if original_content else "[图片]"
            else:
                processed_content = original_content or "[动画表情]"
            # 确保队列清理时不会误删要用的图片，但会在发送后清理
            can_send_messages = True
...
def process_user_messages(user_id):
    ...
    if image_paths:
        logger.info(f"本次消息携带 {len(image_paths)} 张图片/表情，将原生交给 Chat 模型识别。")
        _diag_write("队列取出", f"user={user_id} 图片路径={image_paths}")
    ...
    try:
        ...
        try:
            ...
            if BOMBING_FILE and os.path.exists(BOMBING_FILE):
                wx.SendFiles(filepath=BOMBING_FILE, who=BOMBING_USER)
                logger.info(f"[轰炸] 发送文件给 {BOMBING_USER}: {os.path.basename(BOMBING_FILE)}")
                sent_any = True
            if BOMBING_CONTENT:
                record_sent_text(BOMBING_CONTENT)
                wx.SendMsg(msg=BOMBING_CONTENT, who=BOMBING_USER)
...
_IMAGE_PHRASES = ["生成图片", "生成一张", "生成个", "生成一个", "画一张", "画个", "画一个", "画张", "帮我画", "帮我生成"]


def extract_image_prompt(text, trigger):
    """从消息中提取生图描述"""
    text = (text or "").strip()
    if trigger and text.startswith(trigger):
        return text[len(trigger):].strip()
    for kw in _IMAGE_PHRASES:
...
def generate_image_with_api(prompt):
    ...
    try:
        ...
        response = requests.post(f"{IMAGE_GEN_BASE_URL}/images/generations", headers=headers, json=data, timeout=180)
        response.raise_for_status()
        result = response.json()
        b64 = result['data'][0]['b64_json']
        img_dir = os.path.join(root_dir, '生成图片')
        os.makedirs(img_dir, exist_ok=True)
        path = os.path.join(img_dir, f"gen_{int(time.time())}.png")
        with open(path, 'wb') as f:
            f.write(base64.b64decode(b64))
        return path
    except Exception as e:
        logger.error(f"AI生图调用失败: {e}", exc_info=True)
        return None


def handle_image_generation(msg, who, prompt):
    """处理AI生图请求：提示→生成→发送"""
    try:
        logger.info(f"[AI生图] {who} 需求: {prompt} 模型: {IMAGE_GEN_MODEL}")
        try:
            wx_op_lock.acquire()
            try:
                record_sent_text("正在生成图片，请稍等...")
                wx.SendMsg(msg="正在生成图片，请稍等...", who=who)
            finally:
                wx_op_lock.release()
        except Exception:
            pass
        path = generate_image_with_api(prompt)
        if path:
            wx_op_lock.acquire()
            try:
                wx.SendFiles(filepath=path, who=who)
            finally:
                wx_op_lock.release()
            logger.info(f"[AI生图] 已发送图片给 {who}: {path}")
        else:
            try:
                wx_op_lock.acquire()
                try:
                    record_sent_text("图片生成失败，请稍后再试")
                    wx.SendMsg(msg="图片生成失败，请稍后再试", who=who)
                finally:
                    wx_op_lock.release()
            except Exception:
                pass
    except Exception as e:
        logger.error(f"[AI生图] 处理失败: {e}")
